...
bool sendTable(int &sock, std::string select, std::string startSending)
{
	...
	while ((row = mysql_fetch_row(res)) != nullptr) {
		...
		sstr << (row[0] ? row[0] : NULL);
		for (int i = 1; i < numFields; i++) sstr << '|' << (row[i] ? row[i] : NULL);
...
int server()
{
	...
	if (bind(server, (struct sockaddr*) &addr, sizeof(addr)) == -1) { close(server); DEATH("Bind failed!\n", 2); }
	if (listen(server, 256) == -1) { close(server); DEATH("Listen failed!\n", 3); }
	LOG("Server started\n");
	while (1)
	{
		try {
			...
			socklen_t structClientLen = sizeof(addr);
			int newClient = accept(server, (struct sockaddr*)&clientAddr, &structClientLen);
